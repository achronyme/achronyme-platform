import { dualSimplexEngine, Tableau } from '../../utils/dual-simplex-engine';
...
export function dualSimplexState(): DualSimplexState {
    return {
        isLoading: false,
        errorMessage: null,
        objectiveType: 'max',
        ...
        // El Dual Simplex es útil cuando tenemos restricciones >= que resultan en b_i < 0 después de transformación
        variables: [
            { id: 1, name: 'x1', zCoef: '2' },
            { id: 2, name: 'x2', zCoef: '3' }
        ],
        constraints: [
            {
                id: 1,
        ...
        init() {
            console.log('Dual Simplex Tool Initialized 🚀');
        },
        ...
        solve() {
            ...
            setTimeout(() => {
                try {
                    ...
                    // 2. Llamar al Motor Dual Simplex
                    const result = dualSimplexEngine.solve(objective, this.constraints, this.objectiveType);
    ...
    } as unknown as DualSimplexState;
}

// Registrar globalmente para Alpine
// @ts-ignore
window.dualSimplexState = dualSimplexState;
